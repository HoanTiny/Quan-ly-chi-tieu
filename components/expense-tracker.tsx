...
export default function ExpenseTracker({ userId }: { userId: string }) {
  ...
  const updateExpenseStatus = (id: string, isPaid: boolean) => {
    setExpenses((prevExpenses) => prevExpenses.map((expense) => (expense.id === id ? { ...expense, isPaid } : expense)))
  }
  ...
  const addExpense = async (
    ...
    try {
      ...
      const expenseObj: Expense = {
        ...
        shareMultipliers: expenseShares.reduce(
          (acc, share) => {
            acc[share.roommate_id] = share.multiplier
            return acc
          },
  ...
  const calculateBalances = () => {
    ...
    expenses.forEach((expense) => {
      ...
      sharedWith.forEach((roommateId) => {
        ...
        const amountForRoommate = (expense.amount * roommateParts) / totalMultiplier
      ...
      if (roundedAmount > 0) {
        ...
        const relatedExpenses = expenses
          .filter((expense) => {
            ...
            return expense.sharedWith.includes(debtor.id) && expense.paidBy === creditor.id
          })
          .map((expense) => {
            ...
            const expenseAmount = Math.round((expense.amount * debtorMultiplier) / totalMultiplier)
  ...
  return (
    <>
      ...
      <div className="md:hidden mb-4">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-full">
            <DropdownMenuItem onClick={() => setActiveTab("roommates")}>Thành viên</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setActiveTab("expenses")}>Chi tiêu</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setActiveTab("summary")}>Tổng kết</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setActiveTab("settlement")}>Thanh toán</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setActiveTab("users")}>Người dùng</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setActiveTab("dashboard")}>Báo cáo</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setActiveTab("qrcode")}>Mã QR</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
      ...
      <div className="mt-4">
        ...
        {activeTab === "dashboard" && <MonthlyDashboard expenses={expenses} roommates={roommates} />}
