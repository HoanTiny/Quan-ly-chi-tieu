...
export default function MonthlyDashboard({
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return expenses.filter((expense) => {
      ...
      if (selectedMonth) {
        ...
        withinDateRange = isWithinInterval(new Date(expense.date), {
          start: monthStart,
          end: monthEnd,
        });
      }
  ...
  const netSpendingByRoommate = useMemo(() => {
    ...
    roommates.forEach((roommate) => {
      ...
      const totalReceived = transactionsByReceiver[roommate.id]
        ? transactionsByReceiver[roommate.id].reduce(
            (sum, t) => sum + t.amount,
            0
          )
        : 0;
  ...
  const exportToCSV = () => {
    if (
      ...
      return;
    if (activeTab === 'balance' && monthlySettlementTransactions.length === 0)
      return;
    ...
    if (activeTab === 'overview') {
      ...
      roommates.forEach((roommate) => {
        if (spendingByRoommate[roommate.id] > 0) {
          const monthLabel = selectedMonth
            ? format(parse(selectedMonth, 'yyyy-MM', new Date()), 'MM/yyyy')
            : 'Tất cả';
          csvContent += `"${monthLabel}","${roommate.name}","${
            roommate.room
      ...
      fileName = `bao-cao-chi-tieu-tong-quan-thang-${
        selectedMonth || 'tat-ca'
      }.csv`;
      ...
      filteredExpenses.forEach((expense) => {
        ...
        if (payer) {
          ...
          csvContent += `"${monthLabel}","${payer.name}","${payer.room}","${
            expense.description
          }","${dateLabel}","${formatCurrency(expense.amount)}"\n`;
        }
      });

      fileName = `bao-cao-chi-tieu-chi-tiet-thang-${
        selectedMonth || 'tat-ca'
      }.csv`;
      ...
      roommates.forEach((roommate) => {
        ...
        if (balance !== 0) {
          ...
          csvContent += `"${monthLabel}","${roommate.name}","${
            roommate.room
          }","${formatCurrency(Math.abs(balance))}","${status}"\n`;
        }
      });
      ...
      monthlySettlementTransactions.forEach((transaction) => {
        ...
        if (payer && receiver) {
          csvContent += `"${payer.name}","${payer.room}","${receiver.name}","${
            receiver.room
      ...
      // Net spending export
      csvContent =
        'Tháng,Thành viên,Phòng,Tổng chi tiêu,Tổng được nhận,Tổng phải trả,Chi tiêu thực tế\n';
      ...
      netSpendingByRoommate.forEach((spending) => {
        ...
        if (roommate) {
          ...
          csvContent += `"${monthLabel}","${roommate.name}","${
            roommate.room
          }","${formatCurrency(spending.totalSpent)}","${formatCurrency(
            spending.totalReceived
          )}","${formatCurrency(spending.totalPaid)}","${formatCurrency(
            spending.netSpending
          )}"\n`;
        }
      });

      fileName = `bao-cao-chi-tieu-thuc-te-thang-${
        selectedMonth || 'tat-ca'
      }.csv`;
    }
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col space-y-4 md:space-y-0 md:flex-row items-center justify-between">
        <div>
        ...
        <div className="flex flex-wrap align-center justify-center md:justify-end space-x-0 md:space-x-2 space-y-2 md:space-y-0">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="w-full sm:w-auto">
                <Filter className="h-4 w-4 mr-2 w-full" /> Bộ lọc
              </Button>
            </DropdownMenuTrigger>
      ...
      <CardContent>
        <Tabs
          value={activeTab}
          onValueChange={(value) =>
            setActiveTab(value as 'overview' | 'details' | 'balance' | 'net')
          }
        >
          <TabsList className="mb-4 grid grid-cols-2 sm:grid-cols-4 gap-2 mb-8">
                    ...
                    <CardContent className="p-6">
                      <div className="text-center">
                        ...
                        <p className="text-3xl font-bold mt-2">
                          {formatCurrency(
                            filteredExpenses.length > 0
                              ? totalMonthlySpending / filteredExpenses.length
                              : 0
                          )}
                        </p>
                        <p className="text-sm text-muted-foreground mt-1">
                    ...
                    <div className="border rounded-md overflow-hidden">
                      <Table>
                        ...
                        <TableBody>
                          {roommates
                            .filter(
                              (roommate) => spendingByRoommate[roommate.id] > 0
                            )
                            .sort(
                              (a, b) =>
                                spendingByRoommate[b.id] -
                                spendingByRoommate[a.id]
                            )
                            .map((roommate) => (
                            ...
                            ([category, amount]) => (
                              <TableRow key={category}>
                                ...
                                <TableCell className="text-right">
                                  {totalMonthlySpending > 0
                                    ? `${Math.round(
                                        (amount / totalMonthlySpending) * 100
                                      )}%`
                                    : '0%'}
                                </TableCell>
                              </TableRow>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium">
                    Chi tiết chi tiêu theo thành viên
                  </h3>
                  <div className="text-sm text-muted-foreground">
                    ...
                    {selectedRoommate
                      ? ` • ${
                          roommates.find((r) => r.id === selectedRoommate)
                            ?.name || ''
                        }`
                      : ''}
                  </div>
                ...
                <ScrollArea className="h-[600px] rounded-md border">
                  <div className="p-4">
                      ...
                      {roommates
                        .filter(
                          (roommate) =>
                            ...
                            (!selectedRoommate ||
                              roommate.id === selectedRoommate) &&
                            (!selectedRoom || roommate.room === selectedRoom)
                        )
                        .sort(
                          (a, b) =>
                            spendingByRoommate[b.id] - spendingByRoommate[a.id]
                        )
                        .map((roommate) => (
                    ...
                    {roommates.filter(
                      ...
                      <div className="text-center py-8 text-muted-foreground">
                        Không có dữ liệu chi tiêu nào phù hợp với bộ lọc đã
                        chọn.
                      </div>
                    )}
                  </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex flex-col sm:flex-row justify-between items-center">
                      <div className="flex items-center mb-2 sm:mb-0">
                        ...
                        <span className="text-sm font-medium">
                          Tổng chi tiêu {formatMonth(selectedMonth)}
                        </span>
                      </div>
          ...
          <TabsContent value="balance">
            {monthlySettlementTransactions.length === 0 ? (
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium">
                    Số dư và thanh toán theo tháng
                  </h3>
                  <div className="text-sm text-muted-foreground">
                    {formatMonth(selectedMonth)}
                    {selectedRoom ? ` • Phòng: ${selectedRoom}` : ''}
                  ...
                  <Card>
                    <CardContent className="p-6">
                      <div className="text-center">
                        ...
                        <p className="text-3xl font-bold mt-2">
                          {monthlySettlementTransactions.length}
                        </p>
                        <p className="text-sm text-muted-foreground mt-1">
                          Cần thanh toán
                        </p>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {roommates
                          ...
                          .sort(
                            (a, b) =>
                              balancesByRoommate[b.id] -
                              balancesByRoommate[a.id]
                          )
                          .map((roommate) => {
                            const balance = balancesByRoommate[roommate.id];
                        ...
                        {monthlySettlementTransactions
                          .filter(
                            (transaction) =>
                              ...
                              (!selectedRoom ||
                                roommates.find((r) => r.id === transaction.from)
                                  ?.room === selectedRoom ||
                                roommates.find((r) => r.id === transaction.to)
                                  ?.room === selectedRoom)
                          )
                              ...
                              <TableCell>
                                <div className="flex flex-col">
                                  <span className="font-medium">
                                    {
                                      roommates.find(
                                        (r) => r.id === transaction.from
                                      )?.name
                                    }
                                    {transaction.from === linkedRoommateId &&
                                      ' (Bạn)'}
                                  </span>
                              ...
                              <TableCell>
                                <div className="flex flex-col">
                                  <span className="font-medium">
                                    {
                                      roommates.find(
                                        (r) => r.id === transaction.to
                                      )?.name
                                    }
                                    {transaction.to === linkedRoommateId &&
                                      ' (Bạn)'}
                                  </span>
                                  <span className="text-xs text-muted-foreground">
                                    {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Các khoản thanh toán mỗi người cần thực hiện
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        <div className="space-y-4">
                          ...
                          {roommates.filter(
                            ...
                            <div className="text-center py-4 text-muted-foreground">
                              Không có khoản phải trả nào phù hợp với bộ lọc đã
                              chọn.
                            </div>
                          )}
                        </div>
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Các khoản thanh toán mỗi người sẽ nhận được
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ScrollArea className="h-[300px]">
                        <div className="space-y-4">
                          {roommates
                            .filter(
                              (roommate) =>
                                ...
                                transactionsByReceiver[roommate.id].length >
                                  0 &&
                                (!selectedRoommate ||
                                  roommate.id === selectedRoommate) &&
                                (!selectedRoom ||
                          ...
                          {roommates.filter(
                            ...
                            <div className="text-center py-4 text-muted-foreground">
                              Không có khoản được nhận nào phù hợp với bộ lọc đã
                              chọn.
                            </div>
                          )}
                        </div>
          ...
          <TabsContent value="net">
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">
                  Chi tiêu thực tế theo thành viên
                </h3>
                <div className="text-sm text-muted-foreground">
                  {formatMonth(selectedMonth)}
                  {selectedRoom ? ` • Phòng: ${selectedRoom}` : ''}
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Chi tiêu thực tế = Tổng chi tiêu - Tổng được nhận + Tổng
                    phải trả
                  </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        <TableBody>
                          {netSpendingByRoommate
                            .filter(
                              (spending) =>
                                (!selectedRoommate ||
                                  spending.roommateId === selectedRoommate) &&
                                (!selectedRoom ||
                                  roommates.find(
                                    (r) => r.id === spending.roommateId
                                  )?.room === selectedRoom)
                            )
                            .map((spending) => {
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Phân tích chi tiết các khoản chi tiêu thực tế của mỗi thành
                    viên
                  </CardDescription>
                </CardHeader>
                <CardContent>
                                  ...
                                  <CardContent className="p-0">
                                    ...
                                    <div className="p-4 border-t">
                                      ...
                                      <p className="text-xs text-muted-foreground mt-1">
                                        Tổng chi tiêu - Tổng được nhận + Tổng
                                        phải trả
                                      </p>
                                    </div>
                                  </CardContent>
                        ...
                        <div className="space-y-6">
                          {netSpendingByRoommate
                            ...
                            .map((spending) => {
                              ...
                              const totalNetSpending =
                                netSpendingByRoommate.reduce(
                                  (sum, s) => sum + s.netSpending,
                                  0
                                );
                              const percentage =
                                totalNetSpending > 0
                              ...
                              return (
                                <div key={roommate.id}>
                                  ...
                                  <div className="flex justify-between text-xs mt-1">
                                    <span>
                                      Chi: {formatCurrency(spending.totalSpent)}{' '}
                                      | Nhận:{' '}
                                      {formatCurrency(spending.totalReceived)} |
                                      Trả: {formatCurrency(spending.totalPaid)}
                                    </span>
                ...
                <CardContent className="p-4">
                  <div className="flex flex-col sm:flex-row justify-between items-center">
                    <div className="flex items-center mb-2 sm:mb-0">
                      ...
                      <span className="text-sm font-medium">
                        Tổng chi tiêu thực tế {formatMonth(selectedMonth)}
                      </span>
                    </div>
                    <div className="text-xl font-bold">
                      {formatCurrency(
