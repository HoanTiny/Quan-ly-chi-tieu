...
import { Download, Copy, Check, Trash2, CreditCard, QrCode } from "lucide-react"
...
export default function QRCodeManager({ roommates, householdId, isAdmin, currentUserId }: QRCodeManagerProps) {
  ...
  useEffect(() => {
    const loadQRCodes = async () => {
      ...
      try {
        ...
        const { error: checkError } = await supabase.from("roommate_qrcodes").select("count").limit(1)

        if (checkError && checkError.code === "42P01") {
          // Bảng không tồn tại, gọi API để tạo bảng
          const created = await createQRCodeTableViaAPI()
          if (created) {
            setTableExists(true)
            // Sau khi tạo bảng, thử tải dữ liệu lại
            const { data, error } = await supabase.from("roommate_qrcodes").select("*").eq("household_id", householdId)
            if (!error && data) {
        ...
        if (!isAdmin) {
          ...
          if (!memberError && memberData && memberData.linked_roommate_id) {
            ...
            const linkedRoommateInfo = roommates.find((r) => r.id === linkedRoommateId)
  ...
  const addQRCode = async () => {
    ...
    if (!newQrData.file && !newQrData.accountNumber) {
      toast({
        title: "Thiếu thông tin",
        description: "Vui lòng tải lên hình ảnh QR code hoặc nhập số tài khoản.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (newQrData.file) {
        ...
        try {
          ...
          toast({
            title: "Lỗi",
            description: "Không thể tải lên hình ảnh QR code. Vui lòng thử lại sau.",
            variant: "destructive",
          })
          setIsLoading(false)
          return
      ...
      const { data, error } = await supabase.from("roommate_qrcodes").insert([qrCodeData]).select()
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          qr_label: accountBank
            ? `${accountBank} - ${accountNumber.substring(accountNumber.length - 4)}`
      ...
      if (error) {
        ...
        setQrCodes((prev) => {
          ...
          if (updated[selectedQRCodeForAccount.roommate_id]) {
            updated[selectedQRCodeForAccount.roommate_id] = updated[selectedQRCodeForAccount.roommate_id].map((qr) => {
  ...
  const deleteQRCode = async (qrCode: RoommateQRCode) => {
    ...
    try {
      ...
      if (qrCode.qr_data && qrCode.qr_data.startsWith("qrcodes/")) {
        const { error: storageError } = await supabase.storage.from("qrcodes").remove([qrCode.qr_data])
      ...
      const { error } = await supabase.from("roommate_qrcodes").delete().eq("id", qrCode.id)
      ...
      if (error) {
        ...
        setQrCodes((prev) => {
          ...
          if (updated[qrCode.roommate_id]) {
            updated[qrCode.roommate_id] = updated[qrCode.roommate_id].filter((qr) => qr.id !== qrCode.id)
  ...
  return (
    <Card>
      <CardHeader>
        ...
        {linkedRoommate && (
          <CardDescription>
            Bạn đang được liên kết với thành viên: <span className="font-medium">{linkedRoommate.name}</span>
          </CardDescription>
        )}
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <div className="flex justify-center items-center h-40">Đang tải...</div>
        ) : (
          <Tabs defaultValue="view" className="w-full">
            <TabsList className="grid grid-cols-2 mb-4">
              <TabsTrigger value="view">Xem mã QR</TabsTrigger>
              <TabsTrigger value="manage" disabled={!isAdmin && !linkedRoommate}>
                Quản lý mã QR
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="view">
              <div className="space-y-4">
                {roommates.length === 0 ? (
                  <p className="text-center text-muted-foreground">Chưa có thành viên nào.</p>
                ) : (
                  roommates.map((roommate) => {
                    const roommateQRCodes = qrCodes[roommate.id] || []
                    return (
                      <div key={roommate.id} className="border rounded-md p-4">
                        <h3 className="font-medium text-lg mb-2">{roommate.name}</h3>
                        {roommateQRCodes.length === 0 ? (
                          <p className="text-muted-foreground text-sm">Chưa có mã QR nào.</p>
                        ) : (
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {roommateQRCodes.map((qrCode) => (
                              <div key={qrCode.id} className="border rounded-md p-3 flex flex-col items-center">
                                <div className="mb-2 text-sm font-medium">
                                  {qrCode.qr_label || getQRTypeLabel(qrCode.qr_type)}
                                </div>
                                    ...
                                    <div className="relative">
                                      <p className="font-mono text-lg">
                                        {showAccountNumber[qrCode.id]
                                          ? qrCode.account_number
                                          : qrCode.account_number?.replace(/\d/g, "•")}
                                      </p>
                                      <button
                                        onClick={() => toggleAccountNumberVisibility(qrCode.id)}
                                        className="absolute right-0 top-0 p-1 text-blue-500 hover:text-blue-700"
                                        title={showAccountNumber[qrCode.id] ? "Ẩn số tài khoản" : "Hiện số tài khoản"}
                                      >
                                        {showAccountNumber[qrCode.id] ? "Ẩn" : "Hiện"}
                                      </button>
                                    </div>
                                  </div>
                                ) : null}
                                ...
                                <div className="flex space-x-2 mt-2">
                                  <Button
                                    ...
                                    disabled={!qrCode.qr_image_url && !qrCode.account_number}
                                  >
                                    {copied[qrCode.id] ? (
                                    ...
                                    {copied[qrCode.id] ? "Đã sao chép" : "Sao chép"}
                                  </Button>
                                  {qrCode.qr_image_url && (
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() => downloadQRImage(qrCode.qr_image_url, roommate.name)}
                                    >
                  ...
                  <div className="flex gap-2">
                    <Dialog open={openDialog} onOpenChange={setOpenDialog}>
                      <DialogTrigger asChild>
                        <Button>
                          <QrCode className="h-4 w-4 mr-2" /> Tải lên mã QR
                        </Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogHeader>
                        ...
                        <div className="space-y-4 py-4">
                          {isAdmin && (
                            <div className="space-y-2">
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  const selected = roommates.find((r) => r.id === e.target.value)
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setNewQrData({ ...newQrData, label: e.target.value })}
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="accountNumber">Số tài khoản (tùy chọn)</Label>
                            <Input
                              id="accountNumber"
                              placeholder="Nhập số tài khoản"
                              value={newQrData.accountNumber}
                              onChange={(e) => setNewQrData({ ...newQrData, accountNumber: e.target.value })}
                            />
                            <p className="text-xs text-muted-foreground">
                              Bạn có thể nhập số tài khoản thay vì tải lên hình ảnh QR
                            </p>
                          </div>
                    ...
                    <Button variant="outline" onClick={() => setOpenDialog(true)}>
                      <CreditCard className="h-4 w-4 mr-2" /> Thêm số tài khoản
                    </Button>
                  </div>
                </div>
                      ...
                      return (
                        <div key={roommate.id} className="border rounded-md p-4">
                          ...
                          {roommateQRCodes.length === 0 ? (
                            ...
                            <div className="space-y-2">
                              {roommateQRCodes.map((qrCode) => (
                                <div key={qrCode.id} className="flex justify-between items-center border-b pb-2">
                                      ...
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <Button
                                            variant="ghost"
                                            size="sm"
                                            onClick={() => openAddAccountDialog(qrCode)}
                                          >
      ...
      <Dialog open={openAccountDialog} onOpenChange={setOpenAccountDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={updateAccountNumber} disabled={!accountNumber || isLoading}>
