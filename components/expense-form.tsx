...
export default function ExpenseForm({
  ...
  const [shareMultipliers, setShareMultipliers] = useState<
  ...
  const calculateAmountForRoommate = (roommateId: string) => {
    if (!newExpense.amount || isNaN(Number.parseFloat(newExpense.amount)))
      return 0;
    ...
    onAddExpense(
      {
        ...
        household_id: 'default-household-id', // Replace with the actual household_id value
      },
      expenseShares
    );
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0">
                <CalendarComponent
                  ...
                  onSelect={(date) =>
                    handleExpenseChange('date', date || new Date())
                  }
                  initialFocus
                />
              </PopoverContent>
          ...
          <div className="grid gap-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <Switch
                  id="unequal-sharing"
                  checked={unequalSharing}
                  onCheckedChange={setUnequalSharing}
                />
            ...
            <div className="grid gap-4">
              {roommates.length === 0 ? (
                ...
                rooms.map((room) => {
                  const roommatesInRoom = roommates.filter(
                    (r) => r.room === room
                  );
                      ...
                      {roommatesInRoom.map((roommate) => (
                        ...
                        >
                          <Checkbox
                            ...
                            checked={newExpense.sharedWith.includes(
                              roommate.id
                            )}
                            onCheckedChange={() =>
                              toggleSharedWith(roommate.id)
                            }
                          />
                          <Label
                            htmlFor={`shared-${roommate.id}`}
                            className="flex-grow"
                          >
                            {roommate.name}
                          </Label>
                          ...
                          {unequalSharing &&
                            newExpense.sharedWith.includes(roommate.id) && (
                              <div className="flex items-center space-x-1">
                                <Button
                                  ...
                                  disabled={
                                    (shareMultipliers[roommate.id] || 1) <= 1
                                  }
                                >
                                ...
                                <Button
                                  ...
                                  disabled={
                                    (shareMultipliers[roommate.id] || 1) >= 10
                                  }
                                >
                                  ...
                                  !isNaN(
                                    ...
                                    <span className="text-xs text-muted-foreground ml-2">
                                      ...
                                      {new Intl.NumberFormat('vi-VN', {
                                        ...
                                        calculateAmountForRoommate(roommate.id)
                                      )}
                                    </span>
                                  )}
                              </div>
              ...
              <p className="text-xs text-muted-foreground">
                * Nếu không chọn thành viên nào, chi phí sẽ được chia đều cho
                tất cả thành viên trong cùng phòng với người chi trả.
              </p>
              {unequalSharing && (
                <p className="text-xs text-muted-foreground">
                  * Chế độ chia không đều: Số tiền được chia theo tỷ lệ của hệ
                  số. Ví dụ: nếu có hai người với hệ số là x1 và x2, người thứ
                  nhất sẽ trả 1/3 tổng chi phí và người thứ hai sẽ trả 2/3.
                </p>
              )}
            </div>
          </div>
