...
export default function SummaryView({
  ...
  const [filterDebtDirection, setFilterDebtDirection] = useState<
    'all' | 'from' | 'to'
  >('all');
  ...
  const filteredExpenses = expenses.filter((expense) => {
    ...
    if (searchTerm) {
      ...
      const matchDescription = expense.description
        .toLowerCase()
      ...
      const matchAmount = formatCurrency(expense.amount)
        .toLowerCase()
        .includes(searchLower);
    ...
    if (
      selectedRoommate &&
      expense.paidBy !== selectedRoommate &&
      !expense.sharedWith.includes(selectedRoommate)
    ) {
      return false;
    }
    ...
    if (selectedRoom) {
      ...
      const sharedWithRooms = expense.sharedWith.map(
        (id) => roommates.find((r) => r.id === id)?.room
      );

      if (
        payerRoom !== selectedRoom &&
        !sharedWithRooms.includes(selectedRoom)
      ) {
        return false;
      }
    }
    ...
    if (dateRange.from && new Date(expense.date) < new Date(dateRange.from))
      return false;
    if (dateRange.to) {
  ...
  filteredExpenses.forEach((expense) => {
    ...
    const sharedWith =
      expense.sharedWith.length > 0
        ...
        : roommates
            .filter(
              (r) => r.room === roommates.find((rm) => rm.id === payer)?.room
            )
            .map((r) => r.id);
  ...
  const groupedDebts: Record<
    string,
    { from: Roommate; to: Roommate; amount: number; expenses: string[] }
  > = {};
  ...
  const exportToCSV = () => {
    ...
    // Tạo nội dung CSV
    let csvContent =
      'Người nợ,Phòng,Người được nợ,Phòng,Số tiền,Chi tiêu liên quan\n';
    ...
    const fileName = `tong-ket-chi-tieu${
      dateRange.from ? `-tu-${dateRange.from}` : ''
    }${dateRange.to ? `-den-${dateRange.to}` : ''}.csv`;
    ...
    <Card>
      <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <CardDescription>
            Tổng hợp số dư và các khoản thanh toán cần thực hiện
          </CardDescription>
        </div>
        <div className="flex flex-col sm:flex-row sm:space-x-2 gap-2">
              ...
              <div className="p-2 border-t">
                ...
                <div className="flex flex-col gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      setFilterDebtDirection(
                        e.target.value as 'all' | 'from' | 'to'
                      )
                    }
                  >
                  ...
                  {filterDebtDirection !== 'all' && (
                    <select
                      ...
                      onChange={(e) =>
                        setFilterDebtPerson(e.target.value || null)
                      }
                    >
          ...
          <Button
            ...
            size="sm"
            onClick={exportToCSV}
            disabled={Object.values(groupedDebts).length === 0}
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center p-3 bg-muted rounded-md">
            <span className="font-medium">
              Tổng chi phí{hasActiveFilters ? ' (đã lọc)' : ''}:
            </span>
            <span className="font-bold">
              {formatCurrency(roundToThousand(filteredTotalExpenses))}
            </span>
          </div>
              ...
              <CardContent className="py-2">
                <div className="space-y-2">
                  {roommates
                    .filter((r) => filteredBalances[r.id] > 0)
                    .sort(
                      (a, b) => filteredBalances[b.id] - filteredBalances[a.id]
                    )
                    .map((roommate) => (
                      <div
                        key={roommate.id}
                  ...
                  {roommates.filter((r) => filteredBalances[r.id] > 0)
                    .length === 0 && (
                    <div className="text-center py-2 text-muted-foreground">
                      Không có số dư dương
                    </div>
                  )}
              ...
              <CardContent className="py-2">
                <div className="space-y-2">
                  {roommates
                    .filter((r) => filteredBalances[r.id] < 0)
                    .sort(
                      (a, b) => filteredBalances[a.id] - filteredBalances[b.id]
                    )
                    .map((roommate) => (
                      <div
                        key={roommate.id}
